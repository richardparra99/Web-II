...
module.exports = (app) => {
    ...
    router.post("/", upload.single("imagen"), isJsonRequestValid, validateJson(albumSquema), controller.crearAlbum);
    router.put("/:id", upload.single("imagen"), isJsonRequestValid, validateJson(albumSquema), getObjectOr404(db.album), controller.actualizarAlbumPut);
    router.patch("/:id", upload.single("imagen"), isJsonRequestValid, validateJson(albumOptionalSquema), getObjectOr404(db.album), controller.actualizarAlbumPatch);
