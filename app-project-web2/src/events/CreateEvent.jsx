...
// 👇 URL del backend para poder mostrar la imagen subida
...
const CreateEvent = () => {
    ...
    // 🔹 imagen
    const [posterFile, setPosterFile] = useState(null);        // archivo que se subirá
    const [posterPreviewUrl, setPosterPreviewUrl] = useState(""); // URL para <img>
    ...
    const onFormSubmit = async (e) => {
        ...
        if (posterFile) {
            try {
                ...
                // uploadResp.url → ej: "/uploads/event-posters/abc.jpg"
                const rawUrl = uploadResp.url;

                // guardamos URL absoluta para que el <img> funcione directo
                posterUrlToSave = rawUrl.startsWith("http")
                    ? rawUrl
                    : `${API_BASE_URL}${rawUrl}`;
            } catch (err) {
                console.error(err);
        ...
        const eventData = {
            ...
            startDate, // "YYYY-MM-DD" suficiente para que el backend lo convierta a Date
            capacity: Number(capacity),
            price: price !== "" ? price : null,
            posterUrl: posterUrlToSave, // 👈 aquí va la URL devuelta por el backend
            locationLat: lat,
            locationLng: lng,
        };
